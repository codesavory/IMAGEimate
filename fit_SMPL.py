...
def fit_SMPL(scans, pose_files=None, gender='male', save_path=None, display=None):
    ...
    iterations, pose_iterations, steps_per_iter, pose_steps_per_iter = 3, 2, 30, 30 #default 3,2,30,30
    ...
    for  scan in  scans:
        # Load scan using trumesh
        print("Loading using trimesh")
        #mesh = trimesh.load(scan, process=False)

        print('scan path ...', scan)
        th_scan = tm.from_obj(scan)
        #custom code
        #th_scan.vertices = torch.tensor(mesh.vertices).float()
        #th_scan.faces = torch.tensor(mesh.faces).float()
    ...
    #to get the T-pose fitted SMPL mesh
    # Init SMPL, pose with mean smpl pose, as in ch.registration
    print("Making the fitted SMPL model to T-pose")
    #copy the optimized smpl model and add t-pose
    pose_tpose = torch.zeros((batch_sz, 72))
    smpl_tpose = th_batch_SMPL(batch_sz, smpl.betas, pose_tpose, trans, faces=th_faces).cuda()
    #re-pose it to T-pose
    #import torch.nn as nn
    #smpl_tpose.pose = nn.Parameter(torch.zeros(batch_sz, 72))
    #extract vertices
    verts_tpose, _, _, _ = smpl_tpose()
    th_smpl_meshes_tpose = [tm.from_tensors(vertices=v, faces=smpl_tpose.faces) for v in verts_tpose]
    ...
    if save_path is not None:
        ...
        #save T-pose mesh and pkl
        #save mesh
        save_meshes(th_smpl_meshes_tpose, [join(save_path, n.replace('.obj', '_smpl_tpose.obj')) for n in names])
        # Save params
        for p, b, t, n in zip(smpl_tpose.pose.cpu().detach().numpy(), smpl_tpose.betas.cpu().detach().numpy(),
          smpl_tpose.trans.cpu().detach().numpy(), names):
            smpl_dict = {'pose': p, 'betas': b, 'trans': t}
            pkl.dump(smpl_dict, open(join(save_path, n.replace('.obj', '_smpl_tpose.pkl')), 'wb'))
        print("Done saving fitted T-pose SMPL model")
